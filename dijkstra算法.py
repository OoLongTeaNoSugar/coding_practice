...
class Graph:
    ...
    def add_vertex(self, matrix):

        for i in range(len(matrix)):
            if matrix[i][0] not in self.vertices.keys():
                self.vertices[matrix[i][0]] = {matrix[i][1] : matrix[i][2]} # from : to : val
            else:
                dic = self.vertices[matrix[i][0]]
                dic[matrix[i][1]] = matrix[i][2]
                self.vertices[matrix[i][0]] = dic
...
if __name__ == "__main__":
    ...
    g.add_vertex(matrx)
    # print(g.vertices)
    short, dis = g.get_shortest_path(1, 4)
    print(short, dis)